...
export class FeedsApiService extends AbstractApiService {
    ...
    public setPublic(feedSourceId: string, value: boolean): Observable<IFeedApi> {
        return this.authHttp.put(this.getSecureUrl(this.FEED_URL) + '/' +
    ...
    public setFile(feedSourceId: string, file: File): Observable<any> {
        ...
        return this.uploadService.upload(this.getSecureUrl(this.FEED_VERSION_URL) +
    ...
    public getDownloadUrl(feed: IFeedApi, versionId: string, isPublic: boolean): Observable<string> {
        ...
        return callback.get(url + '/' + (versionId ? versionId : feed.latestVersionId) +
            '/downloadtoken', { headers: this.getHeader() })
            .map(response => response.json())
    ...
    public getFeed(feedSourceId: string, getPublic?: boolean): Promise<any> {
        return getPublic ? this.http.get(this.FEED_URL + '/' + feedSourceId, { headers: this.getHeader() })
            .map(response => response.json()).toPromise() :
            this.authHttp.get(this.getSecureUrl(this.FEED_URL) + '/' +
                feedSourceId, { headers: this.getHeader() }).map(response => response.json()).toPromise();
    }
    ...
    public getNotes(feedSourceId: string, isPublic?: boolean): Promise<any> {
        ...
        return this.authHttp.get(this.getSecureUrl(this.FEED_NOTES) + feedSourceId)
            .map(response => response.json()).toPromise();
    }

    public addNotes(feedSourceId: string, note: string): Observable<any> {
        return this.authHttp.post(this.getSecureUrl(this.FEED_NOTES) + feedSourceId, note)
            .map(response => response.json());
    }

    public getLicenses(): Promise<any> {
        return this.http.get(this.FEED_LICENSE, { headers: this.getHeader() })
            .map(response => response.json()).toPromise();
    }

    public getMiscDatas(): Promise<any> {
        return this.http.get(this.FEED_MISC_DATA, { headers: this.getHeader() })
            .map(response => response.json()).toPromise();
    }
    ...
    public setLicense(feedIds: string[], licenseId: string): Observable<ILicense> {
        ...
        return this.authHttp.put(this.FEED_LICENSE + '/' + licenseId + params, null,
            { headers: this.getHeader(true) }).map(response => response.json());
    }
    ...
    public getFeedVersions(feedSourceId: string, isPublic: boolean): Promise<any> {
        if (isPublic) {
            return this.http.get(this.FEED_VERSION_URL + '?feedSourceId=' + feedSourceId,
                { headers: this.getHeader() }).map(response => response.json()).toPromise();
        } else {
            return this.authHttp.get(this.getSecureUrl(this.FEED_VERSION_URL) +
                '?feedSourceId=' + feedSourceId, { headers: this.getHeader() })
                .map(response => response.json()).toPromise();
        }
    }
    ...
    public getStops(feedId: string, feedVersion: string, isPublic: boolean): Promise<any> {
        ...
        return this.authHttp.get(this.getSecureUrl(this.FEED_STOPS_URL) + '?feedId='
            + feedId, { headers: this.getHeader() }).map(response => response.json()).toPromise();
    }

    public getStop(stopId: string): Promise<any> {
        return this.authHttp.get(this.FEED_STOPS_URL + '/' + stopId + '/',
            { headers: this.getHeader() }).map(response => response.json()).toPromise();
    }
    ...
    public getRoutes(feedId: string, feedVersion: string, isPublic: boolean): Promise<any> {
        ...
        return this.authHttp.get(this.getSecureUrl(this.ROOT_URL) + '/route?feedId='
            + feedId, { headers: this.getHeader() }).map(response => response.json()).toPromise();
    }
    ...
    public getRouteTripPattern(feedId: string, feedVersion: string, routeId: string, isPublic: boolean): Promise<any> {
        if (isPublic) {
            return this.http.get(this.ROOT_URL + '/trippattern?feedId=' + feedId + '&routeId='
                + routeId, { headers: this.getHeader() }).map(response => response.json()).toPromise();
        }
        return this.authHttp.get(this.getSecureUrl(this.ROOT_URL) + '/trippattern?feedId=' +
            feedId + '&routeId=' + routeId, { headers: this.getHeader() }).map(response => response.json()).toPromise();
    }
    public getFeedByVersion(versionId: string, isPublic?: boolean): Promise<any> {
        let url;
        if (isPublic) {
            return this.http.get(this.FEED_VERSION_URL + '/' + versionId, { headers: this.getHeader() })
                .map(response => response.json()).toPromise();
        }
        return this.authHttp.get(this.getSecureUrl(this.FEED_VERSION_URL) + '/' + versionId,
            { headers: this.getHeader() }).map(response => response.json()).toPromise();
    }
}
