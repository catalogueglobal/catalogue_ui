...
export class DatasetsComponent implements AfterViewInit {
  ...
  constructor(protected utils: UtilsService, protected projectsApiService: ProjectsApiService, protected store: Store<DatasetsState>, protected datasetsAction: DatasetsActions, protected config: Configuration, protected feedsApi: FeedsApiService, protected localFilters: LocalFiltersService, actions$: Actions) {
    ...
    actions$.ofType(DatasetsActionType.FEED_CREATE_SUCCESS).subscribe(() =>
      this.store.dispatch(datasetsAction.feedsGet(this.getFeedsParams()))
    );
  }
  ...
  protected downloadFeeds(event: Event) {
    ...
    checkedFeeds.forEach(feed => {
      this.feedsApi.getDownloadUrl(feed).subscribe(url => {
          if (url) {
